...
import { Bot, Radio, Send, Terminal, Eye, Brain, MessageSquare, Users, RefreshCw } from "lucide-react"
...
interface StreamState {
  ...
  thoughts: Array<{ type: string; content: string; time: string }>
...
export default function LiveStreamPage() {
  const [state, setState] = useState<StreamState>({
    terminal: ["\x1b[36m[stream]\x1b[0m Connecting..."],
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('nex-live-stream')
      .on('broadcast', { event: 'update' }, ({ payload }) => {
        const { event, data, time } = payload
        
        setState(prev => {
          switch (event) {
            case "terminal":
              return { ...prev, terminal: [...prev.terminal.slice(-100), data.line] }
            case "preview":
              return { ...prev, preview: data }
            case "thought":
              return { 
                ...prev, 
                thoughts: [...prev.thoughts.slice(-50), { ...data, time }],
                currentThought: ""
              }
            case "thinking":
              return { ...prev, currentThought: data.content }
            case "state":
              return { ...prev, ...data }
            default:
              return prev
  ...
  const sendChat = async () => {
    ...
    await fetch("/api/ai/stream/push", {
      method: "PUT",
    ...
    setChatInput("")
    fetchState() // Refresh to get the new message
  }
  ...
  const thoughtIcons: Record<string, string> = { thinking: "🧠", action: "⚡", insight: "💡" }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white flex flex-col">
      ...
      <header className="border-b border-[#1f1f1f] bg-[#0a0a0a] px-4 h-12 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          {state.isLive ? (
            ...
            <span className="text-[#52525b] text-xs">Offline</span>
          )}
          <span className="text-[#52525b] text-sm hidden sm:inline">Nex Building Live</span>
        </div>
        <div className="flex items-center gap-3 text-sm text-[#52525b]">
          <button onClick={fetchState} className="hover:text-white p-1" title="Refresh">
      ...
      {/* Main Grid */}
      <main className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-2 p-2 min-h-0 overflow-hidden">
        {/* Left: Terminal + Preview */}
        <div className="lg:col-span-8 flex flex-col gap-2 min-h-0 overflow-hidden">
          {/* Terminal */}
          <div className="flex-1 min-h-[200px] bg-[#0d0d0d] rounded-lg border border-[#1f1f1f] flex flex-col overflow-hidden">
            ...
            <div ref={terminalRef} className="flex-1 overflow-y-auto p-3 font-mono text-sm leading-relaxed">
              {state.terminal.map((line, i) => (
                <div key={i} className="whitespace-pre-wrap break-all" dangerouslySetInnerHTML={{ __html: parseAnsi(line) || '&nbsp;' }} />
              ))}
            </div>
          </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-2 min-h-0 overflow-hidden">
          {/* Thoughts */}
          <div className="flex-1 min-h-[150px] bg-[#0d0d0d] rounded-lg border border-[#1f1f1f] flex flex-col overflow-hidden">
            <div className="flex items-center gap-2 px-3 py-2 border-b border-[#1f1f1f] flex-shrink-0">
            ...
            <div ref={thoughtsRef} className="flex-1 overflow-y-auto p-3 space-y-2">
              ...
              {!state.thoughts.length && !state.currentThought && (
                <div className="text-[#52525b] text-sm text-center py-4">Waiting for thoughts...</div>
              )}
            </div>
          </div>
            ...
            <div ref={chatRef} className="flex-1 overflow-y-auto p-3 space-y-2">
              {chat.map((msg) => (
                <div key={msg.id} className="text-sm">
                  <span className={`font-medium ${msg.is_nex ? 'text-lime-400' : 'text-blue-400'}`}>{msg.username}</span>
                  <span className="text-[#52525b] text-xs ml-2">{msg.time}</span>
                  <p className="text-[#a1a1aa] mt-0.5">{msg.message}</p>
                </div>
              ))}
              {!chat.length && <div className="text-[#52525b] text-sm text-center py-4">Chat is open 👋</div>}
            </div>
            ...
            <div className="p-2 border-t border-[#1f1f1f] flex-shrink-0">
              {!username ? (
                <input
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && e.currentTarget.value.trim() && setUsername(e.currentTarget.value.trim())}
                />
              ) : (
                <div className="flex gap-2">
                  ...
                  />
                  <Button size="sm" onClick={sendChat} className="px-3"><Send className="w-4 h-4" /></Button>
                </div>
              )}
            </div>
          </div>
      ...
      <footer className="border-t border-[#1f1f1f] px-4 h-8 flex items-center justify-between text-xs text-[#52525b] flex-shrink-0">
        <span>Built by Nex ⚡</span>
        <span>kulti.club</span>
      </footer>
    </div>
  )
}
